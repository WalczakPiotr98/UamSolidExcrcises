...
public final class Matricies {
	...
	public static double[] euclidean_distances(double[]x, double[][] y, double[] z){
		...
		try{
			double XX = einsum(x);
			distances = Matricies.multiplyByMatrix(y, x);
			distances = Matricies.row_mul(distances, -2);
			distances = Matricies.addValue(distances,  XX);
			distances = Matricies.addMatrixes(distances, z);
		}
	...
	public static double[][] euclidean_distances(double[][] x, double[][] y, double[]z){
		...
		try{
			XX = einsum(x);
			distances = Matricies.multiplyByMatrix(x, Matricies.transpose(y));
			distances = Matricies.row_mul(distances, -2);
			distances = Matricies.addValue(distances, XX);
			distances = Matricies.addValue(distances, z);
		}
		catch(Exception myEx)
        {
			System.out.println("An exception encourred: " + myEx.getMessage());
	...
	public static double[][] cov(double[][]x){
		...
		try{
			temp = Matricies.copy2dArray(x);
			//////////substracting mean //////////////
			X_mean = Statistics.getMean(Matricies.transpose(x));
			for(int j=0;j<x[0].length;j++){
				for(int i=0; i<x.length; i++){
					temp[i][j] -= X_mean[i];
				}
			...
			temp = Matricies.divideByValue(Matricies.multiplyByMatrix(x, Matricies.transpose(temp)), (double)x[0].length-1);
		}
		catch(Exception myEx)
        {
			System.out.println("An exception encourred: " + myEx.getMessage());
	...
	public static double[] logsumexp(double[][] data){
		...
		double[] out = null;
		try{
			double[][] temp = Matricies.transpose(data);
			double[] vmax = Matricies.max(temp, 0);
			out = Matricies.makeLog(Matricies.sum(Matricies.exp(Matricies.substractValue(temp, vmax)), 0));
			out = Matricies.addMatrixes(out, vmax);
			}
		catch(Exception myEx)
        {
            //System.out.println("An exception encourred: " + myEx.getMessage());
