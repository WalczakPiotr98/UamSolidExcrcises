        ...
        for (int i = 0; i < this.M.length; i++) {
            ...
            if (row_energy < 0.0001)
                ...
                while (row_energy > 1.01) {
                    temp_row = Matricies.row_mul(temp_row, 0.99);
                    row_energy = energy(temp_row);
                }
                while (row_energy < 0.99) {
                    temp_row = Matricies.row_mul(temp_row, 1.01);
                    row_energy = energy(temp_row);
                }
            }
            this.M[i] = temp_row;
...
//////////////////////////////////////////////////////////////////////////////////////////
    ...
    private void extract_MFCC() {
        ...
        if (this.samples != null) {
            ...
            for (int i = 0; i < frames_num; i++) {
                ...
                try {
                    frame = Matricies.row_mul(frame, window);
                    ...
                    double[] dot_prod = Matricies.multiplyByMatrix(this.melfb_coeffs, fft_final);
                    for (int j = 0; j < dot_prod.length; j++) {
                        dot_prod[j] = Math.log(dot_prod[j]);
                    }
                    //double[][]D1 = dctmatrix(melfilter_bands);
                    dot_prod = Matricies.multiplyByMatrix(this.D1, dot_prod);
    ...
    private double[][] dctmatrix(int n) {
        ...
        double[][] x = Matricies.meshgrid_ox(n);
        double[][] y = Matricies.meshgrid_oy(n);
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
        ...
        try {
            d1 = Matricies.multiplyMatrixesElByEl(x, y);
        } catch (Exception myEx) {
            myEx.printStackTrace();
            System.exit(1);
        }
